...
public class UserService {
    ...
    public UserResponse getUserStatistics(Long id) {
        ...
        // Add statistics
        long totalBorrows = userRepository.countTotalBorrowsByUserId(id);
        long activeBorrows = userRepository.countActiveBorrowsByUserId(id);
        long overdueBorrows = userRepository.countOverdueBorrowsByUserId(id);
        
        response.setTotalBorrows(totalBorrows);
        response.setActiveBorrows(activeBorrows);
        response.setOverdueBorrows(overdueBorrows);
    ...
    @Transactional(readOnly = true)
    public UserResponse getUserByMoodleId(String moodleId) {
        User user = userRepository.findByMoodleId(moodleId)
                .orElseThrow(() -> new ResourceNotFoundException("User", "moodleId", moodleId));
        return userMapper.toResponse(user);
    }
